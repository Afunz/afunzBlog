      ...
      <Dialog :show="dialogConfig.show"
            ...
            <el-form 
                  ...
                  <el-form-item label="博客摘要" prop="summary" >
                    <el-input v-model="blogFormData.summary" placeholder="请输入摘要" type="textarea" :autosize="{minRows:4,maxRows:4}">
                    </el-input>
                  </el-form-item>
                  <el-form-item label="博客标签" prop="tag" >
                    <div class="tag-input-panel">
                      <div class="tag-list">
                        <el-tag v-for="(item,index) in blogFormData.tag" @close="closeTag(index)" class="tag-item" closable >
                        {{ item }}
                        </el-tag>
                      </div>
                      <span class="info" v-if="blogFormData.tag.length==0">添加标签更容易被搜索引擎收录</span>
                      <span class="iconfont icon-jia" @click="showTagInputHandler" v-if="!showTagInput"></span>
